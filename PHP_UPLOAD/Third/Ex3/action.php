...
foreach($_FILES['file_upload']['name'] as $k=>$v){
	if($v != ''){
		// Check conditions:
		//Check if the file size is between 50KB and 3MB?
				$checkSize = checkSize($_FILES['file_upload']['size'][$k], $min_size, $max_size);
		...
		if($checkSize && $checkExtension){
			...
			//Step 1 : Get file data
				...
				$fileName = $ramdom_string.'_'.$v;
				$tmpName = $_FILES['file_upload']['tmp_name'][$k];
